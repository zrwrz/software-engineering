...
constexpr size_t PASSWORD_MIN_LEN = 6;
...
void ensurePasswordLength(const std::string& password, bool requireMinLength) {
    if (requireMinLength && password.size() < PASSWORD_MIN_LEN) {
        throw std::runtime_error("password length must be at least 6 characters");
    }
    if (password.size() > PASSWORD_MAX_LEN) {
        throw std::runtime_error("password length must be <= 128 characters");
    }
}
...
oatpp::Object<RegisterResponse> UserService::registerUser(
    ...
    ensurePasswordLength(password, true);
...
oatpp::Object<LoginResponse> UserService::login(
    ...
    ensureNotEmpty(username, "username cannot be empty");
    ensureNotEmpty(password, "password cannot be empty");
    ensurePasswordLength(password, false);
...
bool UserService::changePassword(
    ...
    ensurePasswordLength(oldPassword, false);
    ensurePasswordLength(newPassword, true);
