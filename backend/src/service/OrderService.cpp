...
std::string nextOrderNo() {
    ...
    uint32_t seq = counter.fetch_add(1, std::memory_order_relaxed) % 1000;
    std::ostringstream out;
    out << "R" << static_cast<long long>(millis) << std::setw(3) << std::setfill('0') << seq;
    return out.str();
}
...
oatpp::Object<ReservationDetailedResponse> mapReservationDetail(sql::ResultSet* rs) {
    ...
    dto->updatedAt = rs->isNull("updated_at") ? nullptr : oatpp::String(rs->getString("updated_at").c_str());
    return dto;
}
...
oatpp::Object<CreateReservationResponse> OrderService::createReservation(int64_t userId, const oatpp::Object<CreateReservationRequest>& request) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> itemStmt(conn->prepareStatement(
            "SELECT i.available_count, i.status, "
            "COALESCE((SELECT SUM(o.quantity) FROM orders o WHERE o.item_id = i.id AND o.status = 'CREATED'), 0) pending_qty "
            "FROM items i WHERE i.id = ? FOR UPDATE"
        ));
        ...
        const int requestQty = request->quantity.getValue(0);
        const int effectiveAvailable = itemRs->getInt("available_count") - itemRs->getInt("pending_qty");
        if (itemRs->getString("status") != "ON_SHELF" || effectiveAvailable < requestQty) {
            throw std::runtime_error("库存不足或物品不可预约");
        }

        std::unique_ptr<sql::PreparedStatement> creditCheck(conn->prepareStatement(
            "SELECT credit_score, status FROM users WHERE id = ?"
        ));
        ...
        AuditLogRepository::createLog(conn.get(), userId, "ORDER_CREATE", "order:" + std::to_string(orderId), "提交预约申请");
...
oatpp::Object<ReservationDetailedResponse> OrderService::getOrderDetailed(int64_t orderId) {
    ...
    std::unique_ptr<sql::PreparedStatement> stmt(conn->prepareStatement(
        ...
        "DATE_FORMAT(updated_at, '%Y-%m-%d %H:%i:%s') updated_at FROM orders WHERE id = ?"
    ));
...
oatpp::Object<CancelReservationResponse> OrderService::cancelReservation(int64_t currentUserId, int64_t orderId) {
    if (currentUserId <= 0) {
        throw std::runtime_error("用户ID不能为空");
    }
    auto order = getOrderDetailed(orderId);
    if (order->userId.getValue(0) != currentUserId) {
        throw std::runtime_error("仅预约发起人可取消订单");
    }
