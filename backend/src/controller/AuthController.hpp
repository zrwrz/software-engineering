...
class AuthController : public oatpp::web::server::api::ApiController {
  private:
    ...
    void logSuccess(const char* endpoint, const std::string& detail) const {
        OATPP_LOGI("AuthController", "[OK] %s - %s", endpoint, detail.c_str());
    }

    void logFailure(const char* endpoint, const std::string& detail) const {
        OATPP_LOGW("AuthController", "[FAIL] %s - %s", endpoint, detail.c_str());
    }
  ...
  public:
		...
		BODY_DTO(oatpp::Object<RegisterRequest>, request))
	{
		...
		try {
            ...
            logSuccess("POST /api/v1/auth/register",
                "username=" + std::string(request->username ? request->username->c_str() : ""));
			return createOKResponse(Status::CODE_201, response);
		} catch (const std::runtime_error& e) {
            logFailure("POST /api/v1/auth/register", e.what());
		...
		BODY_DTO(oatpp::Object<LoginRequest>, request))
	{
		...
		try {
            ...
            logFailure("POST /api/v1/auth/login", e.what());
			return createErrorResponse(Status::CODE_401, 4001, e.what());
		} catch (const std::exception& e) {
			// 捕获其他未知异常
            logFailure("POST /api/v1/auth/login", e.what());
		...
		HEADER(String, authorization, "Authorization")) 
	{
		...
		try {
            ...
            logFailure("GET /api/v1/users/me", e.what());
        ...
        try {
            ...
            logSuccess("GET /api/v1/users/me", "userId=" + std::to_string(userId));
		...
		BODY_DTO(oatpp::Object<UpdateProfileRequest>, request))
    {
		...
		try {
            ...
            logSuccess("PUT /api/v1/users/me", "userId=" + std::to_string(userId));
			return createOKResponse(Status::CODE_200, updatedUser);
		} catch (const std::runtime_error& e) {
            logFailure("PUT /api/v1/users/me", e.what());
			return createErrorResponse(Status::CODE_401, 4001, e.what());
		} catch (const std::exception& e) {
            logFailure("PUT /api/v1/users/me", e.what());
			return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
		...
		BODY_DTO(oatpp::Object<ChangePasswordRequest>, request))
    {
		...
		try {
            ...
            logSuccess("PUT /api/v1/users/me/password", "userId=" + std::to_string(userId));

			return createOKResponse(Status::CODE_200, success_dto);
		} catch (const std::runtime_error& e) {
            logFailure("PUT /api/v1/users/me/password", e.what());
			return createErrorResponse(Status::CODE_401, 4001, e.what());
		} catch (const std::exception& e) {
			// 捕获其他未知异常
            logFailure("PUT /api/v1/users/me/password", e.what());
