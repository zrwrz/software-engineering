...
class OrderController : public oatpp::web::server::api::ApiController {
  private:
    ...
    void logSuccess(const char* endpoint, const std::string& detail) const {
        OATPP_LOGI("OrderController", "[OK] %s - %s", endpoint, detail.c_str());
    }

    void logFailure(const char* endpoint, const std::string& detail) const {
        OATPP_LOGW("OrderController", "[FAIL] %s - %s", endpoint, detail.c_str());
    }
  ...
  public:
        ...
        BODY_DTO(oatpp::Object<CreateReservationRequest>, request))
    {
        try {
            ...
            AuthHelper::requireAnyRole(context, {"borrower"});
            auto data = OrderService::createReservation(context.userId, request);
            auto itemId = (request && request->itemId) ? request->itemId.getValue(0) : 0;
            logSuccess("POST /api/v1/orders/reservations",
                "userId=" + std::to_string(context.userId) + ", itemId=" + std::to_string(itemId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("POST /api/v1/orders/reservations", e.what());
            return createErrorResponse(Status::CODE_400, 1001, e.what());
        } catch (...) {
            logFailure("POST /api/v1/orders/reservations", "unexpected error");
    ...
    {
        AuthHelper::CurrentUserContext context;
        try {
            context = AuthHelper::parseToken(secret, authorization);
        } catch (const std::runtime_error& e) {
            logFailure("GET /api/v1/orders/my", e.what());
            return createErrorResponse(Status::CODE_401, 1002, e.what());
        }
        try {
            auto data = OrderService::getMyOrders(
                context.userId,
                page ? page.getValue(1) : 1,
                size ? size.getValue(10) : 10,
                status ? status->c_str() : ""
            );
            logSuccess("GET /api/v1/orders/my", "userId=" + std::to_string(context.userId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("GET /api/v1/orders/my", e.what());
            return createErrorResponse(Status::CODE_400, 1001, e.what());
        } catch (...) {
            logFailure("GET /api/v1/orders/my", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        QUERY(String, status, "status"))
    {
        try {
            ...
            auto data = OrderService::getOrderList(
                page ? page.getValue(1) : 1,
                size ? size.getValue(20) : 20,
                status ? status->c_str() : ""
            );
            logSuccess("GET /api/v1/orders", "adminUserId=" + std::to_string(context.userId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("GET /api/v1/orders", e.what());
            return createErrorResponse(Status::CODE_403, 1003, e.what());
        } catch (...) {
            logFailure("GET /api/v1/orders", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        PATH(Int64, orderId, "orderId"))
    {
        try {
            ...
            if (context.role == "borrower" && data->userId.getValue(0) != context.userId) {
                logFailure("GET /api/v1/orders/{orderId}", "borrower forbidden to access order");
                return createErrorResponse(Status::CODE_403, 1003, "无权限操作该订单");
            }
            logSuccess("GET /api/v1/orders/{orderId}",
                "requestUserId=" + std::to_string(context.userId) + ", orderId=" + std::to_string(orderId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("GET /api/v1/orders/{orderId}", e.what());
            return createErrorResponse(Status::CODE_404, 2001, e.what());
        } catch (...) {
            logFailure("GET /api/v1/orders/{orderId}", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        PATH(Int64, orderId, "orderId"))
    {
        try {
            auto context = AuthHelper::parseToken(secret, authorization);
            auto data = OrderService::cancelReservation(context.userId, orderId);
            logSuccess("PATCH /api/v1/orders/{orderId}/cancel",
                "userId=" + std::to_string(context.userId) + ", orderId=" + std::to_string(orderId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("PATCH /api/v1/orders/{orderId}/cancel", e.what());
            return createErrorResponse(Status::CODE_400, 2002, e.what());
        } catch (...) {
            logFailure("PATCH /api/v1/orders/{orderId}/cancel", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        BODY_DTO(oatpp::Object<AuditReservationRequest>, request))
    {
        try {
            ...
            auto data = OrderService::auditReservation(context.userId, orderId, request);
            logSuccess("PATCH /api/v1/orders/{orderId}/audit",
                "adminUserId=" + std::to_string(context.userId) + ", orderId=" + std::to_string(orderId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("PATCH /api/v1/orders/{orderId}/audit", e.what());
            return createErrorResponse(Status::CODE_400, 2002, e.what());
        } catch (...) {
            logFailure("PATCH /api/v1/orders/{orderId}/audit", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        PATH(Int64, orderId, "orderId"))
    {
        try {
            auto context = AuthHelper::parseToken(secret, authorization);
            AuthHelper::requireAnyRole(context, {"resource_admin"});
            auto data = OrderService::registerBorrow(context.userId, orderId);
            logSuccess("PATCH /api/v1/orders/{orderId}/borrow",
                "adminUserId=" + std::to_string(context.userId) + ", orderId=" + std::to_string(orderId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("PATCH /api/v1/orders/{orderId}/borrow", e.what());
            return createErrorResponse(Status::CODE_400, 2002, e.what());
        } catch (...) {
            logFailure("PATCH /api/v1/orders/{orderId}/borrow", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        BODY_DTO(oatpp::Object<RegisterReturnRequest>, request))
    {
        try {
            auto context = AuthHelper::parseToken(secret, authorization);
            AuthHelper::requireAnyRole(context, {"resource_admin"});
            auto data = OrderService::registerReturn(context.userId, orderId, request);
            logSuccess("PATCH /api/v1/orders/{orderId}/return",
                "adminUserId=" + std::to_string(context.userId) + ", orderId=" + std::to_string(orderId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("PATCH /api/v1/orders/{orderId}/return", e.what());
            return createErrorResponse(Status::CODE_400, 2002, e.what());
        } catch (...) {
            logFailure("PATCH /api/v1/orders/{orderId}/return", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
        ...
        HEADER(String, authorization, "Authorization"))
    {
        try {
            auto context = AuthHelper::parseToken(secret, authorization);
            AuthHelper::requireAnyRole(context, {"resource_admin", "system_admin"});
            auto data = OrderService::markOverdue();
            logSuccess("POST /api/v1/internal/orders/mark-overdue", "triggeredBy=" + std::to_string(context.userId));
            return createSuccessResponse(data);
        } catch (const std::runtime_error& e) {
            logFailure("POST /api/v1/internal/orders/mark-overdue", e.what());
            return createErrorResponse(Status::CODE_400, 2002, e.what());
        } catch (...) {
            logFailure("POST /api/v1/internal/orders/mark-overdue", "unexpected error");
            return createErrorResponse(Status::CODE_500, 5000, "系统内部错误");
        }
    }
};
