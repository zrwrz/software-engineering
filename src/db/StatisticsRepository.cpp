...
oatpp::Object<StatsDashboardDto> StatisticsRepository::getDashboard() {
    ...
    dashboard->todayReservationCount = queryInt(conn.get(), "SELECT COUNT(*) FROM orders WHERE DATE(created_at) = CURDATE()");
    dashboard->borrowingCount = queryInt(conn.get(), "SELECT COUNT(*) FROM orders WHERE status = 'BORROWED'");
    dashboard->overdueCount = queryInt(conn.get(), "SELECT COUNT(*) FROM orders WHERE status = 'OVERDUE'");
